       ...
       senTestInvertScope:(BOOL)senTestInvertScope
{
  ...
  // Application tests must be executed in the main thread due to interactions
  // with the Simulator APIs.
  if (isApplicationTest) {
    // Return the block itself so it can be executed later in the main thread.
    return Block_copy(action);
  } else {
    // Execute immediately in this thread if its a logic test, and return a
    // dummy block with the result.
    BOOL success = action();
    return Block_copy(^() { return success; } );
  }
}
  ...
  NSMutableArray *blocks = [NSMutableArray array];
    ...
    for (NSDictionary *testable in testables) {
      NSOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
        ...
        BOOL (^block)() = [self runTestable:testable
                                  reporters:reporters
